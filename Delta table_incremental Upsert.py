...
storage_account_name = "datalakefortestandbox"

# should be at the storage account level and not the container
storage_account_access_key = "EDAVUugd+0zDTDIaQlZOJDMkp1A=="
connection_string = "DefaultEndpointsProtocol=https;AccountName=datalakefortestandbox;AccountKey=6dnEctDSaczagpsJA1y0x34auTz1iA==;EndpointSuffix=core.windows.net"
...
df_test_sample_data = sql("select b.VehicleId as VehicleId, cast(a.da as date) as date,a.sid,round(AVG(a.ld)) as load, b.WheelPosition from Kname.rawtesttelemetry a join Kname.testvehiclemap b on a.sid = b.SensorID  group by a.da,a.sid,b.WheelPosition,b.VehicleId order by VehicleId,a.da,b.WheelPosition")
#df.createOrReplaceTempView("LoadTable")

# COMMAND ----------

# DBTITLE 1,write sample data as a csv file into storage
df_test_sample_data.write.format("csv").save("wasbs://rawstream@datalakefortestandbox.blob.core.windows.net/test/testloaddata_csv.csv")

# COMMAND ----------

# DBTITLE 1,creating table with the csv file
# MAGIC %sql
# MAGIC create table test.testloaddata_csv using csv location "wasbs://rawstream@datalakefortestandbox.blob.core.windows.net/test/testloaddata_csv.csv"
...
df_test_sample_data.write.format("parquet").save("wasbs://rawstream@datalakefortestandbox.blob.core.windows.net/test/testloaddata.parquet")
...
# MAGIC %sql
# MAGIC create table test.testloaddata using parquet location "wasbs://rawstream@datalakefortestandbox.blob.core.windows.net/test/testloaddata.parquet"
...
file_list = dbutils.fs.ls("wasbs://rawstream@datalakefortestandbox.blob.core.windows.net/test")
file_list
...
df_processed_files = spark.read.format("parquet").load("wasbs://rawstream@datalakefortestandbox.blob.core.windows.net/test/filesmetadata.parquet")
...
# DBTITLE 1,this code is to avoid csv folders
# df_tobe_processed = df_tobe_processed.limit(1)
# display(df_tobe_processed)
# df_tobe_processed.count()
...
# filepath = df_tobe_processed.select("filepath").rdd.flatMap(lambda x: x).collect()
# filename = df_tobe_processed.select("filename").rdd.flatMap(lambda x: x).collect()
# filearrivaltime = df_tobe_processed.select("filearrivaltime").rdd.flatMap(lambda x: x).collect()

# from pyspark.sql.functions import lit
# if df_tobe_processed.count() ==1:
#   first_df = spark.read.format("csv").load(filepath,inferschema = True, header = True).withColumn("filename",lit(filename)).withColumn("filereceivedtime",lit(filearrivaltime))
# else:
...
finaldf.createOrReplaceTempView('temptable')

# COMMAND ----------

# MAGIC %sql
# MAGIC MERGE INTO test.testloaddata a
# MAGIC USING temptable b
# MAGIC ON a.VehicleId = b.VehicleId
# MAGIC and a.date = b.date
# MAGIC and a.sid = b.sid
# MAGIC and a.WheelPosition = b.WheelPosition
...
# df_test_data = spark.read.format("parquet").load("wasbs://rawstream@datalakefortestandbox.blob.core.windows.net/test/testloaddata.parquet")
...
#test_final_dataframe.write.format("parquet").mode("overwrite).save("wasbs://rawstream@datalakefortestandbox.blob.core.windows.net/test/testloaddata.parquet")

# COMMAND ----------

# %sql
# Refresh table test.testloaddata
...
df_processed.write.format("parquet").mode("append").save("wasbs://rawstream@datalakefortestandbox.blob.core.windows.net/test/filesmetadata.parquet")
...
df_py.write.format("parquet").save("wasbs://rawstream@datalakefortestandbox.blob.core.windows.net/test/filesmetadata.parquet")

# COMMAND ----------

# MAGIC %sql
# MAGIC create table test.filesmetadata using parquet location "wasbs://rawstream@datalakefortestandbox.blob.core.windows.net/test/filesmetadata.parquet"
...
received_files_list = dbutils.fs.ls("wasbs://rawstream@datalakefortestandbox.blob.core.windows.net/test")
received_files_list
